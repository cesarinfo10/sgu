       ...
       ( select id_asiscapac_actividades_funcionarios_check from asiscapac_actividades_obligatorias_funcionarios
        ...
        (
          ...
          select observacion from asiscapac_actividades_obligatorias_funcionarios 
           ...
           observacion,
          ( 
            select CASE convocado WHEN 't' THEN 'SI' ELSE 'NO' END AS convocado from asiscapac_actividades_obligatorias_funcionarios 
            where ano = $ano 
            and id_asiscapac_actividades = $id_campo_actividades
            and id_usuario = u.id 
          )
            ...
            where u.tipo <> 3 
            and u.activo 
            and u.id_unidad is not null and gu.id = u.id_unidad 
            AND coalesce(u.fecha_desvinculacion::date, (
              SELECT fecha_inicio::date FROM asiscapac_actividades WHERE id = $id_campo_actividades )
               ) >= (SELECT fecha_inicio::date FROM asiscapac_actividades WHERE id = $id_campo_actividades )  
           order by gu.alias, u.apellido,u.nombre";*/
...
<table cellspacing="1" cellpadding="2" class="tabla">
  <tr>
    <td class="texto">
      ...
      Fecha Inicio Actividad : <?php echo($fecha_inicio_actividad); ?>, duración : <?php echo($duracion_actividad); ?> minutos.
    </td>
  </tr>
</table>
