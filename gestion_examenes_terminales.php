...
if (empty($_REQUEST['fecha_ini'])) { $fecha_ini = "$ano-01-01"; }
if (empty($_REQUEST['fecha_fin'])) { $fecha_fin = "$ano-12-31"; }
...
if ($buscar == 'Buscar' && $texto_buscar <> "") {
	...
	if ($ano > 0) { $condicion .= "AND (date_part('year',et.fecha_examen) = $ano) "; }
	...
	if (!empty($fecha_ini) && !empty($fecha_fin)) { $condicion .= " AND et.fecha_examen::date BETWEEN '$fecha_ini'::date AND '$fecha_fin'::date "; }
	...
	//if ($id_carrera <> "") { $cond_estudiantes .= "AND (a.carrera_actual = '$id_carrera') "; }
...
$SQL_exam_term = "SELECT et.id,to_char(fecha_reg,'dd-tmMon-YYYY HH24:MI') AS fecha_reg,
						 ...
						 u.nombre AS ministro_de_fe,($SQL_docentes) AS comision,e.nombre AS escuela,
						 ($SQL_grupo_estud) AS estudiantes
  ...
  <form name="formulario" action="principal.php" method="get">
	...
	<table cellpadding="1" border="0" cellspacing="2" width="auto">
	  ...
	  <td class="celdaFiltro">
		  Año: <br>
		  <select class="filtro" name="ano" onChange="formulario.fecha_ini.value=null; formulario.fecha_fin.value=null; submitform();">
		...
		<td class="celdaFiltro">
          Fecha Examen:<br>
          <input type='date' name='fecha_ini' value="<?php echo($fecha_ini); ?>" class="boton" style='font-size: 9pt' onChange="formulario.fecha_fin.value=this.value; submitform();"> al
