...
$SQL_indicadores_cat = "SELECT gic.id,gic.nombre,gic.alias,gic.descripcion,gic.cod_procedencia,gic.agrupador,gic.subagrupador,
...
if (count($indicadores_cat) > 0) {
	...
	for ($x=0;$x<count($indicadores_cat);$x++) {
		...
		if ($mecanismo_captura=="si") {
			...
			$mecanismo .= "<br><small>{$indicadores_cat[$x]['alias']}</small>";
