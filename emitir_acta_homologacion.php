...
$SQL_prehomo = "SELECT ph.*,m.ano AS ano_malla_nueva,m.carrera AS carrera_nueva,
...
if (count($prehomo) > 0) {
	...
	if (count($prehomo_det) > 0) {
		...
		$SQL_malla_ant = "SELECT vm.carrera AS carrera_antigua,e.nombre AS escuela_antigua,ano AS ano_malla_antigua
