...
$SQL_alumnos = "SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                       ...
                       CASE WHEN ($SQL_cursos_insc)>0 THEN (($SQL_cursos_aprob)::real/($SQL_cursos_insc)::real*100)::numeric(4,1) ELSE 0 END AS tasa_aprobacion_ant $SQL_alumnos_asist
                FROM alumnos AS a
...
$SQL_fecha_ult_mat = "SELECT max(fecha) FROM matriculas WHERE id_alumno=a.id";
...
$SQL_arancel_real = "SELECT monto_arancel*CASE trim(tipo) WHEN 'Semestral' THEN 2 ELSE 1 END AS arancel_real FROM finanzas.contratos WHERE ano=$ANO AND (id_alumno=a.id OR id_pap=a.id_pap) AND estado IN ('E','S','R','A') LIMIT 1";
...
$SQL_alumnos = "SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                       ...
                       CASE WHEN ($SQL_alsies)=1 THEN 'Si' ELSE 'No' END AS al_sies_$ANO_MATRICULA  $SQL_alumnos_asist2
                FROM alumnos AS a
                LEFT JOIN pap ON pap.id=a.id_pap
                LEFT JOIN carreras   AS c ON c.id=a.carrera_actual
...
<?php } ?>
    ...
    <table cellpadding="1" border="0" cellspacing="2" width="auto">
      <tr>
        ...
        <td class="celdaFiltro">
          <div title='Beneficiario Fiscal'>B. F.:</div>
...
<?php if ($modulo == "gestion_alumnos") { ?>
        ...
        <label for="mostrar_asistencia">Asistencia</label>
	    </td>
<?php 	if ($mostrar_asistencia == "si") { ?>
        <td class="celdaFiltro">      
          Periodo asistencia estudiantes:<br>
