COPY (SELECT nro_contrato,fecha_emision,rut,nombre,centro_costo,nro_pagare,
               ...
               FROM (SELECT c.id as nro_contrato,
                      ...
                      ((SELECT sum(monto_pagado) 
                                 ...
                                 WHERE true  AND p.fecha::date BETWEEN '2022-01-01'::date AND '2023-07-14'::date  AND id_glosa>1 AND id_contrato=vc.id
                                )) AS pagos_total,
                      (SELECT sum(pd.monto_pagado) 
                              FROM finanzas.pagos_detalle AS pd 
                              LEFT JOIN finanzas.cobros   AS c on c.id=pd.id_cobro 
                              LEFT JOIN finanzas.pagos    AS p on p.id=pd.id_pago
                              WHERE true  AND p.fecha::date BETWEEN '2022-01-01'::date AND '2023-07-14'::date  AND c.id_contrato=vc.id AND id_glosa IN (2,20)) AS pagos_arancel_pagare,
                      (SELECT sum(pd.monto_pagado) 
                              FROM finanzas.pagos_detalle AS pd 
                              LEFT JOIN finanzas.cobros   AS c on c.id=pd.id_cobro 
                              LEFT JOIN finanzas.pagos    AS p on p.id=pd.id_pago
                              WHERE true  AND p.fecha::date BETWEEN '2022-01-01'::date AND '2023-07-14'::date  AND c.id_contrato=vc.id AND id_glosa IN (21,22)) AS pagos_arancel_cheque,
                      (SELECT sum(pd.monto_pagado) 
                              FROM finanzas.pagos_detalle AS pd 
                              LEFT JOIN finanzas.cobros   AS c on c.id=pd.id_cobro 
                              LEFT JOIN finanzas.pagos    AS p on p.id=pd.id_pago
                              WHERE true  AND p.fecha::date BETWEEN '2022-01-01'::date AND '2023-07-14'::date  AND c.id_contrato=vc.id AND id_glosa IN (3,31)) AS pagos_arancel_contado
               ...
               WHERE true AND c.ano=2023 AND c.estado IS NOT NULL AND (car.regimen = 'PRE')  AND c.fecha::date BETWEEN '2022-01-01'::date AND '2023-07-14'::date 
