...
$SQL_exam_term = "SELECT et.id,to_char(fecha_reg,'dd-tmMon-YYYY HH24:MI') AS fecha_reg,
						 ...
						 u.nombre AS ministro_de_fe,ministro_de_fe_modalidad,e.nombre AS escuela
...
if (count($exam_term) > 0) {
	...
	$SQL_grupo_estud = "SELECT va2.nombre,($SQL_pago_examen) AS pagado
	...
	for ($x=0;$x<count($estudiantes);$x++) {
		...
		$HTML_est .= "<li>{$estudiantes[$x]['nombre']} $pagado</li>";
	}
...
if ($exam_term[0]['estado'] == "Cursado") {
	$boton_reg_notas = "<a href='$enlbase_sm=examenes_terminales_res_calificaciones&id=$id_examen' class='boton'>📋 Registrar Calificaciones</a>";
}
?>
